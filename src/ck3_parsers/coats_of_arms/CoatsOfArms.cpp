...
void CK3::CoatsOfArms::registerKeys()
{
   ...
   registerKeyword("coat_of_arms_manager_database", [this](const std::string&, std::istream& theStream) {
   ...
   for (const auto& coat: coats)
   {
      ...
      if (titleDataItr != titleData.end())
      {
         if (!titleDataItr->second->getCoA())
            throw std::runtime_error("CoA " + std::to_string(coat.first) + " has parent " +
                                     coat.second->getParent()->first + " which has no coat defined!");
         if (!coats.count(titleDataItr->second->getCoA()->first))
            throw std::runtime_error("CoA " + std::to_string(coat.first) + " has parent " +
                                     coat.second->getParent()->first + " which has invalid coat defined!");
         coat.second->loadParent(
             std::make_pair(coat.second->getParent()->first, coats[titleDataItr->second->getCoA()->first]));
         ++counter;
      }
      else
      {
         throw std::runtime_error("CoA " + std::to_string(coat.first) + " has parent " +
                                  coat.second->getParent()->first + " which is undefined!");
      }
   }
