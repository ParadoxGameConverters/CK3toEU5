...
{
   registerKeyword("rotation", [this](const std::string&, std::istream& theStream) {
      rotation = commonItems::singleDouble(theStream).getDouble();
   });
   registerKeyword("depth", [this](const std::string&, std::istream& theStream) {
      depth = commonItems::singleDouble(theStream).getDouble();
   });
   registerKeyword("position", [this](const std::string&, std::istream& theStream) {
      position = commonItems::doubleList(theStream).getDoubles();
   });
   registerKeyword("scale", [this](const std::string&, std::istream& theStream) {
      scale = commonItems::doubleList(theStream).getDoubles();
   });
   registerKeyword("offset", [this](const std::string&, std::istream& theStream) {
