...
void CK3::Religions::registerKeys()
{
   ...
   registerKeyword("religions", [this](const std::string&, std::istream& theStream) {
      religions = Religions(theStream).getReligions();
   });
   registerKeyword("faiths", [this](const std::string&, std::istream& theStream) {
...
void CK3::Religions::linkFaiths(const Faiths& theFaiths)
{
   ...
   for (const auto& religion: religions)
   {
      ...
      for (const auto& faith: religionFaiths)
      {
         ...
         {
            throw std::runtime_error("Religion " + religion.second->getName() + " has faith " +
                                     std::to_string(faith.first) + " which has no definition!");
         }
      }
