...
void CK3::Houses::linkDynasties(const Dynasties& dynasties)
{
   ...
   for (const auto& house: houses)
   {
      ...
      {
         Log(LogLevel::Error) << "House " << std::to_string(house.first) << " has dynasty "
                              << std::to_string(house.second->getDynasty().first) << " which has no definition!";
      }
   }
   Log(LogLevel::Info) << "<> " << counter << " houses updated.";
}
...
void CK3::Houses::importNames(const HouseNameScraper& houseNameScraper)
{
   ...
   {
      if (house->getName().empty() && houseNameScraper.getNameForKey(house->getKey()) &&
          !houseNameScraper.getNameForKey(house->getKey())->empty())
      {
         house->setName(*houseNameScraper.getNameForKey(house->getKey()));
         ++nameCounter;
      }
      if (house->getPrefix().empty() && houseNameScraper.getPrefixForKey(house->getKey()) &&
          !houseNameScraper.getPrefixForKey(house->getKey())->empty())
      {
