...
ck3::DynastiesMap::DynastiesMap(std::istream& input_stream)
{
   ParseDynasties(input_stream);
}

void ck3::DynastiesMap::ParseDynasties(std::istream& input_stream)
{
...
void ck3::DynastiesMap::ParseOnlyDynasties(std::istream& input_stream)
{
   ...
   dynasties_parser.registerRegex(R"(\d+)", [this](const std::string& savegame_dynasty_id, std::istream& input_stream) {
      ...
      if (dynasty_blob_as_string.contains('{'))  // check to filter our inactive dynasties
      {
         ...
         const DynastyParser new_dynasty(dynasty_stream, std::stoll(savegame_dynasty_id));
...
void ck3::DynastiesMap::ParseHouses(std::istream& input_stream)
{
   ...
   house_parser.registerRegex(R"(\d+)", [this](const std::string& house_id, std::istream& input_stream) {
      ...
      if (house_blob_as_string.contains('{'))  // check to filter our inactive houses
      {
         ...
         const HouseParser new_house(house_stream, std::stoll(house_id));
