...
CK3::Titles::Titles(std::istream& theStream)
{
   ...
   // This bit assigns CK3::Level to dynamic titles_ that have a rank definition in the save. It should be all of them,
...
{
   registerKeyword("dynamic_templates", [this](const std::string&, std::istream& theStream) {
      ...
      for (const auto& dynamicTitle: dynamicRanks.getBlobs())
      {
         ...
         if (!dynamicTitleTemplate.getDynamicTitleKey().empty() && !dynamicTitleTemplate.getDynamicTitleRank().empty())
            dynamicTitleRanks.insert(
                std::pair(dynamicTitleTemplate.getDynamicTitleKey(), dynamicTitleTemplate.getDynamicTitleRank()));
      }
   });
   ...
   registerRegex(R"(\d+)", [this](const std::string& ID, std::istream& theStream) {
      // Incoming titles_ may not be actual titles_ but half-deleted junk.
...
void CK3::Titles::linkCoats(const CoatsOfArms& coats)
{
   ...
   for (const auto& title: titles)
   {
      ...
      {
         Log(LogLevel::Warning) << "Title " + title.first + " has CoA " +
                                       std::to_string(title.second->getCoA()->first) +
...
void CK3::Titles::linkTitles()
{
   ...
   std::map<long long, std::map<long long, std::shared_ptr<Title>>>
       dfvRegistry;  // map<liegeTitleID, map<vassalTitleID, vassalTitle>>

   // But before we begin, we need to clear out the mess that is the save. Titles will have defacto liege set to titles_
   // that have no holder. To filter them, let's first run through all titles_ and make a cache of all without a holder.
   ...
   // When a county and a duchy are held by same holder, under a king, both titles_ will have the kingdom set as defacto
   // liege. We need to untangle this mess and drop all vassals held by a person that also holds the vassal's dejure
   // title as a vassal under same liege. :/
   ...
   for (const auto& title: titles)
   {
      ...
      if (title.second->getDFLiege())
      {
         ...
         {
            // fix it immediately.
            Log(LogLevel::Warning) << "Title " + title.first + " has defacto liege " +
                                          std::to_string(title.second->getDFLiege()->first) +
      ...
      if (title.second->getDJLiege())
      {
         ...
         else
         {
            ...
            Log(LogLevel::Warning) << "Title " << title.first << " has DEJURE >>LIEGE<< "
                                   << std::to_string(title.second->getDJLiege()->first)
                                   << " which has no definition! This is not amusing. Rerouting.";
            if (title.second->getDFLiege())
            {
               if (const auto& dfLiegeItr = IDCache.find(title.second->getDFLiege()->first);
                   dfLiegeItr != IDCache.end())
               {
                  // We're rerouting title to be dejure under dfliege, whomever this is. The can cause baronies to be
                  // dejure under empires, but see if I care at this point.
                  title.second->loadDJLiege(*dfLiegeItr);
                  Log(LogLevel::Warning) << "Rerouted " << title.first << " to dejure liege "
                                         << dfLiegeItr->second->getName();
               }
               else
               {
      ...
      if (!title.second->getDJVassals().empty())
      {
         ...
         for (const auto& dJVassal: title.second->getDJVassals())
         {
            ...
            else
            {
               ...
               Log(LogLevel::Warning) << "Title " << title.first << " has DEJURE vassal "
                                      << std::to_string(dJVassal.first) << " which has no definition! Amusing.";
            }
         }
   ...
   // Finally update the dfvassal lists of all titles_ that gained new vassals.
   ...
   Log(LogLevel::Info) << "<> " << DFLcounter << " defacto lieges, " << DJLcounter << " dejure lieges, " << DFVcounter
                       << " defacto vassals, " << DJVcounter << " dejure vassals updated.";
}
...
void CK3::Titles::linkCharacters(const Characters& characters)
{
   ...
   for (const auto& title: titles)
   {
      ...
      if (title.second->getHolder())
      {
         ...
         {
            Log(LogLevel::Error) << "For heaven's sake, title " + title.first + " has holder " +
                                        std::to_string(title.second->getHolder()->first) +
                                        " who is dead or has no definition!";
            // Attempt recovery. Since titles_ are still unlinked we'll have to iterate to find DFliege's holder
            // manually.
            if (title.second->getDFLiege())
            {
   ...
   Log(LogLevel::Info) << "<> " << holderCounter << " holders, " << claimantCounter << " claimants, " << heirCounter
                       << " heirs, " << exCounter << " previous holders, " << electorCounter << " electors updated.";
}

void CK3::Titles::linkLandedTitles(const LandedTitles& landedTitles)
{
   // We're injecting clay towards titles_ instead the other way around.
   auto counter = 0;
