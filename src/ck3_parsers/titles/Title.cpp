...
{
   nameParser.registerKeyword("name", [this](const std::string&, std::istream& theStream) {
   ...
   nameParser.registerKeyword("custom", [this](const std::string&, std::istream& theStream) {
      // Only present when the player manually renamed the title (custom="Malik City").
      customName = commonItems::singleString(theStream).getString();
   });
   nameParser.registerKeyword("adj", [this](const std::string&, std::istream& theStream) {
      adjective = commonItems::singleString(theStream).getString();
   });
   nameParser.registerKeyword("date", [this](const std::string&, std::istream& theStream) {
      creationDate = date(commonItems::singleString(theStream).getString());
   });
   nameParser.registerKeyword("title_history_names", [this](const std::string&, std::istream& theStream) {
   ...
   registerKeyword("title_name_data", [this](const std::string&, std::istream& theStream) {
      nameParser.parseStream(theStream);
   });
   registerKeyword("key", [this](const std::string&, std::istream& theStream) {
   ...
   registerKeyword("adj", [this](const std::string&, std::istream& theStream) {
      adjective = commonItems::singleString(theStream).getString();
   });
   /*
    // We're commenting this out because they started shoving in full utf-8 mapnames which are incompatible with eu4
   when non-ascii.
    // retaining code just in case if needed alter down the line.

   registerKeyword("specific_title_name", [this](const std::string&, std::istream& theStream) {
           // This will override name set previously.
           displayName = commonItems::singleString(theStream).getString();

           // This is a check for ascii characters actually. We may have chinese here. However, in english they start
   with lowercase "the". if (!displayName.empty() && std::islower(displayName[0])) displayName[0] =
   std::toupper(displayName[0]);
   ...
   */
   ...
   registerKeyword("coat_of_arms_id", [this](const std::string&, std::istream& theStream) {
      coa = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
   });
   registerKeyword("succession_election", [this](const std::string&, std::istream& theStream) {
      const auto newTitle = Title(theStream, 0);
      electors = newTitle.getElectors();
   });
   registerKeyword("electors", [this](const std::string&, std::istream& theStream) {
      for (auto electorID: commonItems::intList(theStream).getInts())
         electors.insert(std::pair(electorID, nullptr));
   });
   registerKeyword("landless", [this](const std::string&, std::istream& theStream) {
      landless = commonItems::singleString(theStream).getString() == "yes";
   });
   registerKeyword("color", [this](const std::string&, std::istream& theStream) {
      color = laFabricaDeColor.getColor(theStream);
   });
   registerKeyword("history", [this](const std::string&, std::istream& theStream) {
...
{
   auto counter = 0;
   // For CK3 flagging dejure alone is not enough as we'll be losing a ton of land when duchies break away in full
   // independence. Flag our dejure AND defacto counties, or our dejure/defacto vassals' counties, wherever those may
   // be. Actual flagging will happen on county level as higher titles_ have no dejure provinces linked to them at this
   // stage. We're leaving baronies out of this, counties are sufficient.
...
void CK3::Title::pickDisplayName(const std::map<std::string, std::shared_ptr<Title>>& possibleTitles)
{
   ...
   if (myDuchyCapital->isRenamed() && possibleTitles.contains(myDuchyCapital->getName()) &&
...
std::shared_ptr<CK3::Title> CK3::Title::findDuchyCapital()
{
   ...
   if (myDuchy->getDJVassals().empty() || !myDuchy->getDJVassals().contains(capID) ||
       !myDuchy->getDJVassals().at(capID))
      return nullptr;
...
CK3::LEVEL CK3::Title::getLevel() const
{
   ...
   // If not...
   // This is the questionable part. It should work for customs as they are treated identically as any other - with
   // dejure parents and all. exceptions are dynamic mercs, landless religious orders and similar - but those don't hold
   ...
   if (djLiege)
      ...
      return LEVEL::EMPIRE;  // If this is wrong for landless mercs, it won't affect anything as they are landless
                             // anyway.
}
...
{
   // We're reconstructing defacto hierarchy; we're redirecting our DFvassals from our DFtitle into our DJtitle - if
   // holder of this title holds hold both. This is important - otherwise secondary titles_ wouldn't have their own
   // vassals under them but under primary title, breaking PU splitoffs, and resulting with those secondary
   // duchies/kingdoms being discarded as landless.
   ...
   for (const auto& [dfVassalID, dfVassal]: dfVassals)
   {
      ...
      if (dfVassal->areDFLiegeAndDJLiegeSet() &&
          !dfVassal->areDFLiegeAndDJLiegeSame())  // We have different df and dj lieges there.
      {
         // Do we own both titles_?
         if (doesHolderHaveCharacterDomain())
         {
            const auto holderTitles = holder->second->GetCharacterRealm()->getDomain();

            // Make a small cache of the holder's owned title names.
            const auto titleNameCache = getTitleNamesFromHolderDomain();

            if (titleNameCache.contains(dfVassal->getDFLiege()->second->getName()) &&
                titleNameCache.contains(dfVassal->getDJLiege()->second->getName()))
            {
...
{
   return isHolderSet() && isHolderLinked() && holder->second->GetCharacterRealm();
}
