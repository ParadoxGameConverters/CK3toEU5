...
// landlessness, and most importantly relation between baronies and barony provinces so we can link titles_ to actual
// clay. Since titles_ are nested according to hierarchy we do this recursively.

// Keep in mind that we use this class as middleware between titles_ and hard geographical data. Baronies and Counties
// have said data, but newfangled custom empires and such found in Titles will not be present here. They should have
// colors defined in their Title block anyway, and whatever relates to Title over there takes precedence over data in
// this class.
...
void CK3::LandedTitles::registerKeys()
{
   registerRegex(R"((h|e|k|d|c|b)_[A-Za-z0-9_\-\']+)", [this](const std::string& titleName, std::istream& theStream) {
      // Pull the titles_ beneath this one and add them to the lot, overwriting existing ones.
   ...
   registerKeyword("definite_form", [this](const std::string&, std::istream& theStream) {
   ...
   registerKeyword("province", [this](const std::string&, std::istream& theStream) {
      province = std::make_pair(commonItems::singleInt(theStream).getInt(), nullptr);
   });
   registerKeyword("can_be_named_after_dynasty", [this](const std::string&, std::istream& theStream) {
      mayBeNamedAfterDynasty = commonItems::singleString(theStream).getString() == "yes";
   });
   registerRegex(commonItems::catchallRegex, commonItems::ignoreItem);
}

void CK3::LandedTitles::linkProvinceHoldings(const ProvinceHoldings& province_holdings_)
{
   // We're linking provinces into BARONY titles_ only, as other titles_ have no holding data.
   auto counter = 0;
   const auto& provinceData = province_holdings_.getProvinceHoldings();
   for (const auto& landedTitle: foundTitles)
   {
      ...
      {
         throw std::runtime_error("Landed title " + landedTitle.first + " has province holding " +
                                  std::to_string(landedTitle.second->getProvince()->first) +
...
{
   // We're linking county details into COUNTY titles_ only, as other titles_ have no such details.
   auto counter = 0;
   ...
   for (const auto& landedTitle: foundTitles)
   {
      ...
      {
         // Nothing. People with missing titles_ from mods and whatnot can have holes on their map.
