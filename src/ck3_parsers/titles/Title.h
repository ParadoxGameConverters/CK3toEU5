...
class Title: commonItems::parser
{
  public:
   ...
   void pickDisplayName(const std::map<std::string, std::shared_ptr<Title>>&
           possibleTitles);                    // Grants one county's name to another during N:1/N:M mappings
  ...
  private:
   ...
   std::pair<long long, std::shared_ptr<Title>>
       capital;              // capital title is a COUNTY, even for county itself and baronies beneath it!
   ...
   std::optional<std::string> customName;  // "Malik City" - only present when the player manually renamed the title.
   ...
   date creationDate;  // Date of the last holder change - the current holder's accession. 9999.1.1 for never-held
                       // titles_.
   ...
   std::vector<std::pair<long long, std::shared_ptr<Character>>>
       heirs;  // Order of heirs is unclear so we're keeping them ordered and using first if able.
   std::map<long long, std::shared_ptr<Character>>
       claimants;  // People holding a claim to this title. Incredibly useful.
   std::map<long long, std::shared_ptr<Character>>
       electors;                  // People involved in elections regardless of election type law.
   ...
   std::shared_ptr<LandedTitles>
       clay;  // Middleware towards geographical data, essential for b_&c_, potentially obsolete for others.
   ...
   std::map<std::string, std::shared_ptr<Title>>
       ownedDFCounties;  // used to map higher-lvl titles_ directly to clay. Includes self! Every c_+ title has this.
   std::map<std::string, std::shared_ptr<Title>> ownedDJCounties;  // ditto
   std::optional<std::pair<std::string, std::shared_ptr<Title>>>
       generatedLiege;                                              // Liege we set manually while splitting vassals.
   std::map<std::string, std::shared_ptr<Title>> generatedVassals;  // Vassals we split off deliberately.
   std::pair<std::string, std::shared_ptr<Title>>
       holdingTitle;  // topmost owner title (e_francia or similar), only c_s have this.
   ...
   std::optional<LEVEL>
       dynamicLevel;  // Maybe assigned through dynamic ranks, otherwise getLevel will try to guesstimate.
};
}  // namespace CK3
