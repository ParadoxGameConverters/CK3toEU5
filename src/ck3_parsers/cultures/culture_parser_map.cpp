#include "culture_parser_map.hpp"
...
ck3::CultureParserMap::CultureParserMap(std::istream& input_stream)
{
   ParseCultureManager(input_stream);
}

void ck3::CultureParserMap::ParseCultureManager(std::istream& input_stream)
{
...
void ck3::CultureParserMap::ParseCultures(std::istream& input_stream)
{
   ...
   cultures_parser.registerRegex(R"(\d+)", [this](const std::string& culture_id, std::istream& input_stream) {
      const auto new_culture = CultureParser(input_stream, std::stoll(culture_id));
