...
{
	registerKeyword("list", [this](const std::string&, std::istream& theStream) {
	...
	registerKeyword("item", [this](const std::string&, std::istream& theStream) {
		const auto scraper = Flags(theStream);
		const auto& foundFlags = scraper.getFlags();
		flags.insert(foundFlags.begin(), foundFlags.end());
	});
	registerKeyword("flag", [this](const std::string&, std::istream& theStream) {
