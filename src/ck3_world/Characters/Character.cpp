...
{
	registerKeyword("first_name", [this](const std::string&, std::istream& theStream) {
		name = commonItems::singleString(theStream).getString();
	});
	registerKeyword("birth", [this](const std::string&, std::istream& theStream) {
		birthDate = date(commonItems::singleString(theStream).getString());
	});
	registerKeyword("dead_data", [this](const std::string&, std::istream& theStream) {
		const auto tempBlock = Character(theStream, charID);
		deathDate = tempBlock.getDeathDate();
		dead = true;
	});
	registerKeyword("date", [this](const std::string&, std::istream& theStream) {
		deathDate = date(commonItems::singleString(theStream).getString());
	});
	registerKeyword("culture", [this](const std::string&, std::istream& theStream) {
		culture = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
		if (culture->first == 4294967295)
			culture.reset();
	});
	registerKeyword("faith", [this](const std::string&, std::istream& theStream) {
		faith = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
		if (faith->first == 4294967295)
			faith.reset();
	});
	registerKeyword("dynasty_house", [this](const std::string&, std::istream& theStream) {
		house = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
	});
	registerKeyword("skill", [this](const std::string&, std::istream& theStream) {
	...
	registerKeyword("traits", [this](const std::string&, std::istream& theStream) {
		for (const auto traitID: commonItems::intList(theStream).getInts())
			traits.insert(std::pair(traitID, std::string()));
	});
	registerKeyword("alive_data", [this](const std::string&, std::istream& theStream) {
		const auto tempBlock = Character(theStream, charID);
	...
	registerKeyword("piety", [this](const std::string&, std::istream& theStream) {
		const auto tempBlock = Character(theStream, charID);
		piety = tempBlock.getAccumulated();
	});
	registerKeyword("prestige", [this](const std::string&, std::istream& theStream) {
		const auto tempBlock = Character(theStream, charID);
		prestige = tempBlock.getAccumulated();
	});
	registerKeyword("accumulated", [this](const std::string&, std::istream& theStream) {
		accumulated = commonItems::singleDouble(theStream).getDouble();
	});
	registerKeyword("obedience_target", [this](const std::string&, std::istream& theStream) {
		suzerain = std::make_pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
	});
	registerKeyword("gold", [this](const std::string&, std::istream& theStream) {
		goldParser.parseStream(theStream);
	});
	registerKeyword("court_data", [this](const std::string&, std::istream& theStream) {
		const auto tempBlock = Character(theStream, charID);
		employer = tempBlock.getEmployer();
		knight = tempBlock.isKnight();
		councilor = tempBlock.isCouncilor();
	});
	registerKeyword("employer", [this](const std::string&, std::istream& theStream) {
		employer = std::make_pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
	});
	registerKeyword("knight", [this](const std::string&, std::istream& theStream) {
		knight = commonItems::singleString(theStream).getString() == "yes";
	});
	registerRegex("council_task|special_council_tasks", [this](const std::string&, std::istream& theStream) {
		councilor = true;
		commonItems::ignoreItem("unused", theStream);
	});
	registerKeyword("female", [this](const std::string&, std::istream& theStream) {
		female = commonItems::singleString(theStream).getString() == "yes";
	});
	registerKeyword("landed_data", [this](const std::string&, std::istream& theStream) {
		characterDomain = CharacterDomain(theStream);
	});
	registerKeyword("family_data", [this](const std::string&, std::istream& theStream) {
		const auto tempBlock = Character(theStream, charID);
		spouse = tempBlock.getSpouse();
		children = tempBlock.getChildren();
	});
	registerKeyword("primary_spouse", [this](const std::string&, std::istream& theStream) {
		spouse = std::make_pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
	});
	registerKeyword("child", [this](const std::string&, std::istream& theStream) {
		for (const auto childID: commonItems::llongList(theStream).getLlongs())
			children.emplace_back(childID, nullptr);
	});
	...
	registerKeyword("title", [this](const std::string&, std::istream& theStream) {
		tempTitle = commonItems::singleLlong(theStream).getLlong();
	});
	registerRegex(commonItems::catchallRegex, commonItems::ignoreItem);

	goldParser.registerKeyword("value", [this](const std::string&, std::istream& theStream) {
