...
{
	registerKeyword("is_powerful_vassal", [this](const std::string&, std::istream& theStream) {
		powerfulVassal = commonItems::singleString(theStream).getString() == "yes";
	});
	registerKeyword("vassal_power_value", [this](const std::string&, std::istream& theStream) {
		vassalStrength = commonItems::singleDouble(theStream).getDouble();
	});
	registerKeyword("government", [this](const std::string&, std::istream& theStream) {
	...
	registerKeyword("realm_capital", [this](const std::string&, std::istream& theStream) {
		realmCapital = std::make_pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
	});
	registerKeyword("domain", [this](const std::string&, std::istream& theStream) {
