...
void CK3::Characters::linkFamilies()
{
	auto childCounter = 0;
	auto parentCounter = 0;
	for (const auto& character: characters)
	{
		std::vector<std::pair<long long, std::shared_ptr<Character>>> linkedChildren;
		for (const auto& child: character.second->getChildren())
		{
			const auto& childItr = characters.find(child.first);
			if (childItr == characters.end())
				continue; // Pruned from the save; nothing to link.
			linkedChildren.emplace_back(*childItr);
			if (character.second->isFemale())
				childItr->second->loadMother(std::pair(character.first, character.second));
			else
				childItr->second->loadFather(std::pair(character.first, character.second));
			++parentCounter;
		}
		childCounter += static_cast<int>(linkedChildren.size());
		character.second->loadChildren(linkedChildren);
	}
	Log(LogLevel::Info) << "<> " << childCounter << " children linked to " << parentCounter << " parental bonds.";
}
