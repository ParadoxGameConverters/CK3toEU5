...
class Character: commonItems::parser
{
  public:
	...
	[[nodiscard]] const auto& getKnights() const { return knights; }
	...
	void loadChildren(const std::vector<std::pair<long long, std::shared_ptr<Character>>>& theChildren) { children = theChildren; }
	void loadFather(const std::pair<long long, std::shared_ptr<Character>>& theFather) { father = theFather; }
	void loadMother(const std::pair<long long, std::shared_ptr<Character>>& theMother) { mother = theMother; }
	...
	void loadKnights(const std::map<long long, std::shared_ptr<Character>>& newKnights) { knights = newKnights; }
  ...
  private:
	...
	std::vector<std::pair<long long, std::shared_ptr<Character>>> children;
	std::optional<std::pair<long long, std::shared_ptr<Character>>> father; // Established through linkFamilies from the parents' child lists.
	std::optional<std::pair<long long, std::shared_ptr<Character>>> mother;
	...
	std::map<long long, std::shared_ptr<Character>> knights; // employed champions and commanders, not council staff
