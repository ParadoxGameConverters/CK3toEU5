...
{
	registerKeyword("pattern", [this](const std::string&, std::istream& theStream) {
		pattern = commonItems::singleString(theStream).getString();
	});
	registerKeyword("color1", [this](const std::string&, std::istream& theStream) {
		try
		{
	...
	registerKeyword("color2", [this](const std::string&, std::istream& theStream) {
		try
		{
	...
	registerKeyword("color3", [this](const std::string&, std::istream& theStream) {
		try
		{
	...
	registerKeyword("textured_emblem", [this](const std::string&, std::istream& theStream) {
		texturedEmblems.emplace_back(Emblem(theStream));
	});
	registerKeyword("colored_emblem", [this](const std::string&, std::istream& theStream) {
		coloredEmblems.emplace_back(Emblem(theStream));
	});
	registerKeyword("sub", [this](const std::string&, std::istream& theStream) {
		subs.emplace_back(std::make_shared<CoatOfArms>(theStream, 0));
	});
	registerKeyword("instance", [this](const std::string&, std::istream& theStream) {
		auto instance = EmblemInstance(theStream);
		if (instance.getOffset().empty())
			instance.defaultOffset();
		instances.emplace_back(instance);
	});
	registerKeyword("parent", [this](const std::string&, std::istream& theStream) {
