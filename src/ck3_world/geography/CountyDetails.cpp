...
void CK3::CountyDetails::registerKeys()
{
   ...
   registerKeyword("counties", [this](const std::string&, std::istream& theStream) {
...
void CK3::CountyDetails::linkCultures(const Cultures& cultures)
{
   ...
   for (const auto& countyDetail: countyDetails)
   {
      ...
      {
         throw std::runtime_error("County " + countyDetail.first + " has culture " +
                                  std::to_string(countyDetail.second->getCulture().first) +
...
void CK3::CountyDetails::linkFaiths(const Faiths& faiths)
{
   ...
   for (const auto& countyDetail: countyDetails)
   {
      ...
      {
         throw std::runtime_error("County " + countyDetail.first + " has faith " +
                                  std::to_string(countyDetail.second->getFaith().first) + " which has no definition!");
      }
   }
   Log(LogLevel::Info) << "<> " << counter << " counties updated.";
}
